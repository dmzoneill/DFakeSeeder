...
from d_fake_seeder.lib.component.notebook import Notebook
...
class TestNotebook(unittest.TestCase):
    ...
    def test_init(self, mock_logger_info):
        ...
        self.assertIsNotNone(notebook.torrents_columnview)
        self.assertIsNotNone(notebook.peers_columnview)
    ...
    def test_get_selected_torrent(self, mock_logger_info):
        ...
        notebook.torrents_columnview = MagicMock()
        notebook.model = MagicMock()
        notebook.model.torrent_list = [MagicMock(id=1), MagicMock(id=2)]
        notebook.torrents_columnview.get_selection().get_selected.return_value = (  # noqa
            None,
            None,
        )
    ...
    def test_update_notebook_peers(self, mock_logger_info):
        notebook = Notebook(MagicMock(), MagicMock())
        notebook.peers_columnview = MagicMock()
        ...
        self.assertTrue(notebook.peers_columnview.set_model.called)
        self.assertTrue(notebook.peers_columnview.get_model.called)
        self.assertEqual(
            len(notebook.peers_columnview.get_model().append.call_args_list), 2
        )

        # Case where store already has rows
        store = MagicMock()
        store.clear = MagicMock()
        notebook.peers_columnview.get_model.return_value = store
        notebook.model.get_liststore_item.return_value.get_seeder.return_value.peers = (
            [  # noqa
                1,
                2,
        ...
        self.assertEqual(
            len(notebook.peers_columnview.get_model().append.call_args_list), 3
        )
