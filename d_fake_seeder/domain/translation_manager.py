...
class TranslationManager:
    ...
    def __init__(
        ...
        Args:
            ...
            supported_languages: Set of supported language codes
                (default: auto-discovered from localedir)
    ...
    def _discover_supported_languages(self) -> Set[str]:
        ...
        try:
            ...
            logger.debug(
                f"Discovered languages: {sorted(discovered_languages)} " f"(took {discovery_time:.1f}ms)",
                "TranslationManager",
            )
        except Exception:
            logger.debug(
                "Warning: Could not discover languages from ...: ...",
                "TranslationManager",
            )
    ...
    def _is_valid_language_code(self, code: str) -> bool:
        ...
        Returns:
            True if it looks like a language code
            (2-3 lowercase letters, optionally with region)
        """
        import re
    ...
    def switch_language(self, lang_code: str) -> str:
        ...
        with logger.performance.operation_context("switch_language", "TranslationManager"):
            ...
            if self.current_language == lang_code:
                logger.debug(
                    f"Already using language '{lang_code}' - skipping switch",
                    "TranslationManager",
                )
                return lang_code
            logger.debug(
            ...
            if lang_code not in self.supported_languages:
                logger.debug(
                    f"Warning: Unsupported language '{lang_code}', falling back to system locale",
                    "TranslationManager",
                )
            ...
            with logger.performance.operation_context("gettext_translation", "TranslationManager"):
                try:
                    ...
                    logger.debug(
                        f"Warning: Translation file for '{lang_code}' not found, " f"falling back to system language",
                        "TranslationManager",
                    )
        ...
        if self.translatable_widgets:
            with logger.performance.operation_context("widget_refresh", "TranslationManager"):
                logger.debug(
                    f"Starting widget refresh for {len(self.translatable_widgets)} widgets",
                    "TranslationManager",
                )
        ...
        if self.translatable_menus:
            with logger.performance.operation_context("menu_refresh", "TranslationManager"):
                logger.debug(
                    f"Starting menu refresh for {len(self.translatable_menus)} menus",
                    "TranslationManager",
                )
    ...
    def scan_builder_widgets(self, builder: Gtk.Builder):
        ...
        if builder_id in self._scanned_builders:
            logger.debug(
                "Skipping scan_builder_widgets - builder {builder_id} already scanned",
                "TranslationManager",
            )
            return
    ...
    def _refresh_translations_immediate(self):
        ...
        try:
            logger.debug(
                "Starting immediate refresh for {len(self.translatable_widgets)} widgets",
                "TranslationManager",
            )
            for i, widget_info in enumerate(self.translatable_widgets):
                ...
                try:
                    ...
                    if i % 50 == 0 or i < 10:
                        logger.debug(
                            f"Widget {i}/{len(self.translatable_widgets)}: '{translation_key}' -> '{translated_text}'",
                            "TranslationManager",
                        )
                except Exception as e:
                    logger.debug(
            ...
            logger.debug("Translation refresh completed", "TranslationManager")
        finally:
            self._updating = False
    ...
    def _set_widget_property(self, widget: Gtk.Widget, property_name: str, value: str):
        ...
        if property_name == "label":
            ...
            try:
                ...
                logger.debug(
                    f"Warning: Could not set property '{property_name}' on {widget.__class__.__name__}: {e}",
                    "TranslationManager",
                )
    ...
    def _recreate_menu(self, menu_info: Dict[str, Any]):
        ...
        for item in items:
            ...
            logger.debug(
                f"Menu item: '{item['key']}' -> '{translated_text}' (action: {item['action']})",
                "TranslationManager",
            )
