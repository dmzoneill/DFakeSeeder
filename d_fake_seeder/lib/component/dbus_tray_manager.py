"""
Clean D-Bus Tray Manager for DFakeSeeder

Manages GTK3 tray subprocess via D-Bus communication WITHOUT any GTK/GI imports
in the main process to avoid GTK version conflicts.
"""
...
from lib.component.dbus_tray_server_simple import TrayDBusServer
from lib.logger import logger


class DBusTrayManager:
    """
    Clean D-Bus-based tray manager that spawns a GTK3 subprocess for system tray.

    This version completely avoids GTK/GI imports in the main GTK4 process to
    ...
    def is_available(self) -> bool:
        ...
        """
        try:
            # Check if D-Bus is available (basic import test)
            import dbus  # noqa: F401

            logger.debug("D-Bus python bindings available", extra={"class_name": self.__class__.__name__})
            ...
            # Test if the subprocess can run (quick test without starting the full tray)
            try:
                test_cmd = ["python3", script_path, "--test-mode"]
                result = subprocess.run(test_cmd, capture_output=True, timeout=5, text=True)
                if result.returncode == 0:
                    logger.debug("GTK3 tray subprocess test passed", extra={"class_name": self.__class__.__name__})
                    return True
                else:
                    logger.warning(
                        f"GTK3 tray subprocess test failed: {result.stderr}",
                        extra={"class_name": self.__class__.__name__},
                    )
                    return False
            except (subprocess.TimeoutExpired, FileNotFoundError) as e:
                logger.warning(f"GTK3 tray subprocess test error: {e}", extra={"class_name": self.__class__.__name__})
                return False
