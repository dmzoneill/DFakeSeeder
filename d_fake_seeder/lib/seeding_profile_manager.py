...
class SeedingProfileManager:
    ...
    def get_profile_settings(self, profile_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if profile_name == "custom":
                ...
                custom_settings = self.app_settings.get("custom_seeding_profile",
                                                       predefined_profiles["balanced"].copy())
                ...
                self.logger.warning(f"Unknown profile '{profile_name}', using balanced",
                                  extra={"class_name": self.__class__.__name__})
                return predefined_profiles["balanced"]

        except Exception as e:
            self.logger.error(f"Error getting profile settings for '{profile_name}': {e}",
                            extra={"class_name": self.__class__.__name__})
    ...
    def apply_profile(self, profile_name: str) -> bool:
        ...
        """
        try:
            self.logger.info(f"Applying seeding profile: {profile_name}",
                           extra={"class_name": self.__class__.__name__})
            ...
            if not self.validate_profile(profile_settings):
                self.logger.error(f"Profile '{profile_name}' failed validation",
                                extra={"class_name": self.__class__.__name__})
                return False
            ...
            self.logger.info(f"Successfully applied seeding profile: {profile_name}",
                           extra={"class_name": self.__class__.__name__})
            return True
    ...
    def _apply_profile_settings(self, profile_settings: Dict[str, Any]) -> None:
        ...
        for profile_key, settings_key in setting_mappings.items():
            if profile_key in profile_settings:
                ...
                self.logger.debug(f"Applied {settings_key} = {value}",
                                extra={"class_name": self.__class__.__name__})

        # Store profile-specific settings
        profile_specific_keys = [
            "share_ratio_target", "idle_probability", "speed_variance",
            "burst_probability", "throttle_mode", "client_behavior"
        ]
            ...
            for setting_name, validator in validations:
                if setting_name in profile_settings:
                    ...
                    if not validator(value):
                        self.logger.error(f"Invalid value for {setting_name}: {value}",
                                        extra={"class_name": self.__class__.__name__})
                        return False
    ...
    def create_custom_profile(self, base_template: str = "balanced") -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Created custom profile from {base_template} template",
                           extra={"class_name": self.__class__.__name__})
            return custom_settings
    ...
    def get_profile_summary(self, profile_name: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"Error generating profile summary for '{profile_name}': {e}",
                            extra={"class_name": self.__class__.__name__})
